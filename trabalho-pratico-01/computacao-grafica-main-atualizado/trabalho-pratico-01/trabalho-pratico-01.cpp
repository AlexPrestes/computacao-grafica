...
class Enfermeiro : public Object {
public:
    Enfermeiro(){
    ...
    this->insertVertex({0.13, -0.18, 0.0});
    this->insertVertex({0.15, -0.48, 0.0});
    this->insertBuild({GL_TRIANGLE_STRIP, num_vertices, 4, {0.8, 1, 1, 1}});

    this->insertVertex({-0.145, -0.48, 0.0});
    this->insertVertex({-0.155, -0.58, 0.0});
    this->insertVertex({0.145, -0.48, 0.0});
    this->insertVertex({0.155, -0.58, 0.0});
    this->insertBuild({GL_TRIANGLE_STRIP, num_vertices+4, 4, {0.8, 1, 1, 1}});
...
void scene_event(GLFWwindow *window, Scene *scene){
    ...
    if(glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS){
        scene->currentObject()->translate( { 0.00, 0.001, 0.00} );
    }
    ...
    if(glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS){
        scene->currentObject()->scale( { 0.0005, 0.0005, 0.0005} );
    }
...
int main(){
    ...
    Coronga coronga5;
    Coronga coronga6;
    coronga.translate({0.0, 0.85, 0.0});
    coronga2.translate({0.0, 0.55, 0.0});
    coronga3.translate({0.0, 0.25, 0.0});
    coronga4.translate({0.0, -0.25, 0.0});
    coronga5.translate({0.0, -0.55, 0.0});
    coronga6.translate({0.0, -0.85, 0.0});

    coronga.scale({-0.85, -0.85, 0.0});
    coronga2.scale({-0.85, -0.85, 0.0});
    coronga3.scale({-0.85, -0.85, 0.0});
    coronga4.scale({-0.85, -0.85, 0.0});
    coronga5.scale({-0.85, -0.85, 0.0});
    coronga6.scale({-0.85, -0.85, 0.0});

    // Adicionando Humano
    Humano humano;
    Enfermeiro enfermeiro;
    humano.translate({-0.6, 0.6, 0.0});
    enfermeiro.translate({0.6, 0.6, 0.0});
    

    Seringa seringa;
    seringa.translate({0.3, 0.0, 0.0});
    seringa.scale({-0.5, -0.5, 0.0});

    Frasco frasco;
    frasco.translate({0.6, -0.6, 0.0});
    frasco.scale({-0.5, -0.5, 0.0});

    // Vetor de poligonos que serão renderizados
    Scene cena1(program);
    Scene cena2(program);
    cena1.insertObject(coronga);
    cena1.insertObject(coronga2);
    cena1.insertObject(coronga3);
    cena1.insertObject(coronga4);
    cena1.insertObject(coronga5);
    cena1.insertObject(coronga6);
    cena1.insertObject(humano);
    cena1.insertObject(enfermeiro);
